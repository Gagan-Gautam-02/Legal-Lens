...
import { analyzeTos, type AnalysisResult, type Conversation } from '@/app/actions';
import { Loader2, LogOut, FileUp, FileText, History, Briefcase, Languages } from 'lucide-react';
...
interface AnalysisHistoryItem {
  ...
  createdAt: Timestamp;
  tos: string;
  // The full analysis is now expected
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    let unsubscribe = () => {};
    if (user && selectedHistory) {
      const q = query(collection(db, 'users', user.uid, 'history', selectedHistory.id, 'conversations'), orderBy('createdAt', 'asc'));
    ...
    return () => unsubscribe();
  }, [user, analysisId, selectedHistory]);


  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: { tos: '', language: 'English' }
  });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    const result = await analyzeTos(values.tos, values.language);
  ...
  const handleHistoryClick = async (item: AnalysisHistoryItem) => {
    if (!user) return;
    // Clear the current live analysis
    setAnalysis(null);
    setAnalysisId(null);
    setConversationHistory([]);

    // Fetch the full document from firestore to ensure we have all data
    ...
    if (docSnap.exists()) {
        ...
        if (item.createdAt) {
          ...
          toast({ title: 'Loaded History', description: 'Displaying a past analysis.' });
        }
    } else {
        toast({ variant: 'destructive', title: 'Error', description: 'Could not find the selected analysis history.' });
    }
  }
            ...
            {selectedHistory && user && (
              <div>
                 <div className="mb-4 flex justify-between items-center">
                    <h2 className="text-2xl font-bold font-headline text-primary">Viewing Past Analysis</h2>
                ...
                <AnalysisDisplay 
                  ...
                  tosDocument={selectedHistory.tos} 
                  userId={user.uid}
                  analysisId={selectedHistory.id}
                  conversationHistory={conversationHistory}
                />
              </div>
            )}

            {!analysis && !isLoading && !selectedHistory && (
                <div className="flex flex-col items-center justify-center h-full rounded-lg border border-dashed p-8 text-center bg-card min-h-[300px]">
