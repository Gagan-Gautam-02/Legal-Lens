...
export async function analyzeTos(
  ...
  if (!language) {
    return { error: 'Please select a language for the analysis.' };
  }

  try {
    const [summary, keyClauses, riskGaps] = await Promise.all([
      summarizeTermsOfService({ termsOfService: tosDocument, language }),
      identifyKeyClauses({ tosDocument, language }),
      analyzeRiskGaps({ tosDocument, language }),
    ]);
